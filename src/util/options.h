...
static inline bool sc_record_format_is_audio_only(enum sc_record_format fmt)
{
	return fmt == SC_RECORD_FORMAT_M4A || fmt == SC_RECORD_FORMAT_MKA || fmt == SC_RECORD_FORMAT_OPUS ||
	       fmt == SC_RECORD_FORMAT_AAC || fmt == SC_RECORD_FORMAT_FLAC || fmt == SC_RECORD_FORMAT_WAV;
}
...
static inline bool sc_orientation_is_mirror(enum sc_orientation orientation)
{
...
static inline bool sc_orientation_is_swap(enum sc_orientation orientation)
{
	assert(!(orientation & ~7));
	return orientation & 1;
}

constexpr sc_orientation sc_orientation_get_rotation(sc_orientation orientation) noexcept
{
	assert((static_cast<int>(orientation) & ~7) == 0);
	return static_cast<sc_orientation>(static_cast<int>(orientation) & 3);
}

enum sc_orientation sc_orientation_apply(enum sc_orientation src, enum sc_orientation transform);

static inline const char *sc_orientation_get_name(enum sc_orientation orientation)
{
	switch (orientation) {
