...
size_t sc_adb_devices_select(const sc_vec_adb_devices &devices, size_t len, const sc_adb_device_selector &selector,
			     size_t *idx_out)
{
	size_t count = 0;
...
bool sc_adb_accept_device(const sc_adb_device &device, const sc_adb_device_selector &selector)
{
	switch (selector.type) {
	case SC_ADB_DEVICE_SELECT_ALL:
		return true;
