...
static bool
sc_demuxer_recv_codec_id(sc_demuxer& demuxer, uint32_t* codec_id) {
    uint8_t data[4];
...
void sc_demuxer::init(const char* name, sc_socket socket, std::shared_ptr<sc_demuxer_callbacks> callbacks, void* cbs_userdata)
{
...
bool sc_demuxer::start()
{
    ...
    bool ok = sc_thread_create(this->thread, run_demuxer, "scrcpy-demuxer",
        this);
    if (!ok) {
